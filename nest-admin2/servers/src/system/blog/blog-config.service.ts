...
export class BlogConfigService {
  ...
  async updateConfig(categories: Array<{ id: string; name: string; slug: string }>, tags: Array<{ id: string; name: string; slug: string }>): Promise<ResultData> {
    ...
    if (config) {
      ...
      config.tags = tags;
      const res = await this.blogConfigManager.transaction(async (transactionalEntityManager) => {
        return await transactionalEntityManager.save<BlogConfigEntity>(config);
      });
      ...
      const res = await this.blogConfigManager.transaction(async (transactionalEntityManager) => {
        return await transactionalEntityManager.save<BlogConfigEntity>(newConfig);
      });
