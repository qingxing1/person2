...
import { IsString, IsEmail, IsNotEmpty, MaxLength, IsOptional, IsEnum, IsNumber } from 'class-validator'
...
export class UpdateUserMessageDto {
  ...
  @ApiProperty({ description: '消息主题', example: '网站反馈建议', required: false })
  @IsString({ message: 'subject 类型错误，正确类型 string' })
  @IsOptional()
  @MaxLength(100, { message: '消息主题最多100个字符' })
  subject?: string

  @ApiProperty({ description: '消息内容', example: '这是一个测试消息内容...', required: false })
  ...
  @ApiProperty({ description: '消息状态', example: 'read', enum: ['unread', 'read'], required: false })
