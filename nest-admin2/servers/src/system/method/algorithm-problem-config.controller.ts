...
export class AlgorithmProblemConfigController {
  constructor(private readonly algorithmConfigService: AlgorithmProblemConfigService) {}
  ...
  @ApiResult(AlgorithmProblemConfigEntity)
  async updateConfig(@Body('categories') categories: Array<{ id: string; name: string; slug: string }>): Promise<ResultData> {
