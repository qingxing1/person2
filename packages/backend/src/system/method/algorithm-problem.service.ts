...
export class AlgorithmProblemService {
  ...
  async create(dto: CreateAlgorithmProblemDto): Promise<ResultData> {
    ...
    const res = await this.algorithmProblemManager.transaction(
      async (transactionalEntityManager) => {
        return await transactionalEntityManager.save<AlgorithmProblemEntity>(
          algorithmProblem
        );
      }
    );
  ...
  async update(
    id: number,
    dto: UpdateAlgorithmProblemDto
  ): Promise<ResultData> {
    const existing = await this.algorithmProblemRepo.findOne({ where: { id } });
    if (!existing)
      return ResultData.fail(
        AppHttpCode.DATA_IS_EMPTY,
        "算法问题不存在或已被删除"
      );

    const { affected } = await this.algorithmProblemManager.transaction(
      async (transactionalEntityManager) => {
        return await transactionalEntityManager.update<AlgorithmProblemEntity>(
          AlgorithmProblemEntity,
          id,
          dto
        );
      }
  ...
  async delete(id: number): Promise<ResultData> {
    ...
    const { affected } = await this.algorithmProblemManager.transaction(
      async (transactionalEntityManager) => {
        return await transactionalEntityManager.delete<AlgorithmProblemEntity>(
          AlgorithmProblemEntity,
          id
        );
      }
