...
export class AlgorithmProblemController {
  constructor(
    private readonly algorithmProblemService: AlgorithmProblemService
  ) {}
  ...
  async update(
    @Param("id") id: number,
    @Body() dto: UpdateAlgorithmProblemDto
  ): Promise<ResultData> {
  ...
  async findAll(
    ...
    return this.algorithmProblemService.findAll({
      title,
      category,
      difficulty,
    });
