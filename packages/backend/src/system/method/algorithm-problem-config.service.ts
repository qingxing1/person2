...
export class AlgorithmProblemConfigService {
  ...
  async updateConfig(
    ...
    if (config) {
      ...
      const res = await this.algorithmConfigManager.transaction(
        async (transactionalEntityManager) => {
          return await transactionalEntityManager.save<AlgorithmProblemConfigEntity>(
            config
          );
        }
      );
      ...
      // 创建配置
      const newConfig = plainToInstance(AlgorithmProblemConfigEntity, {
        categories,
      });
      const res = await this.algorithmConfigManager.transaction(
        async (transactionalEntityManager) => {
