...
export class DeptService {
  ...
  async create(dto: CreateDeptDto): Promise<ResultData> {
    ...
    const res = await this.deptManager.transaction(
  ...
  async update(dto: UpdateDeptDto): Promise<ResultData> {
    ...
    const { affected } = await this.deptManager.transaction(
      async (transactionalEntityManager) => {
        return await transactionalEntityManager.update<DeptEntity>(
          DeptEntity,
          dto.id,
          dto
        );
  ...
  async delete(id: string): Promise<ResultData> {
    ...
    const { affected } = await this.deptManager.transaction(
      async (transactionalEntityManager) => {
        await transactionalEntityManager.delete<DeptEntity>(DeptEntity, {
          parentId: id,
        });
        return await transactionalEntityManager.delete<DeptEntity>(
          DeptEntity,
          id
        );
      }
