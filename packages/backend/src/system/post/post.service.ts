...
export class PostService {
  ...
  async create(dto: CreatePostDto): Promise<ResultData> {
    ...
    if (existing)
      return ResultData.fail(
        ...
        "当前岗位名称与编码已存在，请修改后重新创建"
      );
    const post = plainToInstance(PostEntity, dto);
    const res = await this.postManager.transaction(
  ...
  async update(dto: UpdatePostDto): Promise<ResultData> {
    ...
    if (!existing)
      return ResultData.fail(
        AppHttpCode.POST_NOT_FOUND,
        "岗位不存在或已被删除，请修改后重新添加"
      );
    const { affected } = await this.postManager.transaction(
      async (transactionalEntityManager) => {
        return await transactionalEntityManager.update<PostEntity>(
          PostEntity,
          dto.id,
          dto
        );
  ...
  async delete(id: string): Promise<ResultData> {
    ...
    const { affected } = await this.postManager.transaction(
      async (transactionalEntityManager) => {
        return await transactionalEntityManager.delete<PostEntity>(
          PostEntity,
          id
        );
      }
