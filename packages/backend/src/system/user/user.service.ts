...
export class UserService {
  ...
  async importUsers(file: Express.Multer.File): Promise<ResultData> {
    ...
    if (!acceptFileType.indexOf(file.mimetype))
      return ResultData.fail(
        AppHttpCode.FILE_TYPE_ERROR,
        "文件类型错误，请上传 .xls 或 .xlsx 文件"
      );
    if (file.size > 5 * 1024 * 1024)
      return ResultData.fail(
        AppHttpCode.FILE_SIZE_EXCEED_LIMIT,
        "文件大小超过，最大支持 5M"
      );
    ...
    if (
      accountErrArr.length > 0 ||
      phoneErrArr.length > 0 ||
      emailErrArr.length > 0
    ) {
      return ResultData.fail(
  ...
  async updateStatus(
    ...
    if (!existing)
      ResultData.fail(AppHttpCode.USER_NOT_FOUND, "当前用户不存在或已删除");
    ...
    if (!affected)
      ResultData.fail(AppHttpCode.SERVICE_ERROR, "更新失败，请稍后尝试");
    await this.redisService.hmset(
      getRedisKey(RedisKeyPrefix.USER_INFO, userId),
      { status }
    );
    return ResultData.ok();
  }
  ...
  async createOrUpdateUserRole(
    ...
    const res = await this.userManager.transaction(
      async (transactionalEntityManager) => {
