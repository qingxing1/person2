...
export class MessageController {
  ...
  @ApiQuery({ name: 'search', required: false, type: String, description: '统一搜索关键字（匹配姓名/邮箱/主题）' })
  @ApiQuery({ name: 'status', required: false, enum: ['unread', 'read'], description: '消息状态' })
  @ApiQuery({ name: 'startDate', required: false, type: String, description: '开始时间，格式：YYYY-MM-DD' })
  @ApiQuery({ name: 'endDate', required: false, type: String, description: '结束时间，格式：YYYY-MM-DD' })
