...
export class MessageService {
  ...
  async findAll(queryDto: FindMessageListDto): Promise<ResultData> {
    try {
      ...
      let queryBuilder =
        this.userMessageRepository.createQueryBuilder("message");
      if (search) {
        // 使用OR条件组合搜索
      ...
      if (status) {
        queryBuilder = queryBuilder.andWhere("message.status = :status", {
          status,
        });
      }
      ...
      if (endDate) {
        ...
        queryBuilder = queryBuilder.andWhere("message.submitTime < :endDate", {
          endDate: end,
        });
      }
  ...
  async update(updateDto: UpdateUserMessageDto): Promise<ResultData> {
    try {
      ...
      const existing = await this.userMessageRepository.findOne({
        where: { id },
      });
      if (!existing) {
        return ResultData.fail(404, "消息不存在");
