...
export class PermService {
  ...
  async findUserMenus(
    ...
    const menus = menusResult.map((v) =>
      objAttrToCamelOrUnderline(
        v,
        "camelCase",
        UserType.SUPER_ADMIN ? "" : "m_"
      )
    );
    await this.redisService.set(
      redisKey,
    ...
    while (_cursor != "0") {
      const [cursor, elements] = await this.redisService
        ...
        .scan(
          _cursor || "0",
          "MATCH",
          match || "nest:user:[menu|role|perm]*",
          "COUNT",
          this.TRAVERSE_MAX_VALUE
        );
      const _elements = !this.REDIS_PREFIX
        ? elements
        : elements.map((ele) => ele.replace(this.REDIS_PREFIX, ""));
      keys.push(..._elements);
      _cursor = cursor;
    }
    return keys;
  ...
  async findAppAllRoutesBySwaggerApi(): Promise<RouteDto[]> {
    ...
    const { data } = await lastValueFrom(
      this.http.get(
        `http://localhost:${this.config.get("app.port")}/api/docs-json`
      )
    );
    const routes = [];
    if (data?.paths) {
