        ...
        if (result) {
          ...
          if (user.type === UserType.ORDINARY_USER) {
            ...
            await transactionalEntityManager.save<UserRoleEntity>(
              plainToInstance(UserRoleEntity, userRole)
            );
          }
        }
        return result;
  ...
  async delete(id: string): Promise<ResultData> {
    ...
    const { affected } = await this.roleManager.transaction(
      async (transactionalEntityManager) => {
        ...
        const result = await transactionalEntityManager.delete<RoleEntity>(
          RoleEntity,
          id
        );
        return result;
