...
export default () => {
  ...
  if (env === 'docker' || env === 'production') {
    config.db.mysql.password = process.env.MYSQL_ROOT_PASSWORD || config.db.mysql.password
    config.app.port = Number(process.env.APP_PORT) || config.app.port
    config.app.file.location = process.env.UPLOAD_LOCATION || config.app.file.location
  }
